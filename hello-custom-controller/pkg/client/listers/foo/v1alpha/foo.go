...
type FooLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha.Foo, err error)
...
// List lists all Foos in the indexer.
func (s *fooLister) List(selector labels.Selector) (ret []*v1alpha.Foo, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha.Foo))
	})
	return ret, err
}
...
type FooNamespaceLister interface {
	...
	Get(name string) (*v1alpha.Foo, error)
...
// List lists all Foos in the indexer for a given namespace.
func (s fooNamespaceLister) List(selector labels.Selector) (ret []*v1alpha.Foo, err error) {
...
// Get retrieves the Foo from the indexer for a given namespace and name.
func (s fooNamespaceLister) Get(name string) (*v1alpha.Foo, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha.Resource("foo"), name)
	}
	return obj.(*v1alpha.Foo), nil
}
